...
plot(NA,NA,type="l",xlim=range(x), ylim = range(c(y1,y2,y3)), ylab="f(x)", col="red", main="Dichten von Normalverteilungen")
lines(x,y1, col="red")
lines(x,y2, col="blue")
...
p <- NULL
for (rdx in 1:length(row)){
  for (cdx in 1:length(col)){
    p <- c(p,pnorm(row[rdx]+col[cdx]))
  }
}

m <- matrix(p, nrow=length(row), ncol=length(col), byrow = T)
colnames(m) <- col
rownames(m) <- row

knitr::kable(m, digits=4)
```
...
\bigskip
- **Wahrscheinlichkeitsfunktion:** $f(x)=\frac{\lambda^x}{k!}e^{-\lambda}$
